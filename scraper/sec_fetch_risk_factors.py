...
def load_companies(path: Path) -> List[dict]:
    ...
    for row in rows:
        ...
        row.setdefault("company_layer", "")
        row.setdefault("company_notes", row.get("notes", ""))
    return rows
...
def resolve_sec_entry(ticker: str, ticker_map: Dict[str, dict]) -> Tuple[Optional[str], Optional[dict]]:
...
def main() -> int:
    ...
    for company in companies:
        ...
        company_layer = (company.get("company_layer") or "").strip().lower()
        ...
        prior_ticker = seen_ciks.get(cik)
        if prior_ticker:
            print(
